...
BOOL ConnectWebSocketNonBlocking(void)
{
	...
	if (connect(g_websocket, result->ai_addr, (int)result->ai_addrlen) == SOCKET_ERROR)
	{
		...
		{
			CString errorMsg;
			errorMsg.Format(_T("WebSocket connect failed: %d"), error);
			LogError(errorMsg);
			freeaddrinfo(result);
			closesocket(g_websocket);
			g_websocket = INVALID_SOCKET;
		...
		if (result <= 0)
		{
			LogError(_T("WebSocket connection timeout"));
			freeaddrinfo(result);
			closesocket(g_websocket);
			g_websocket = INVALID_SOCKET;
			WSACleanup();
		...
		if (getsockopt(g_websocket, SOL_SOCKET, SO_ERROR, (char*)&error_code, &len) != 0 || error_code != 0)
		{
			LogError(_T("WebSocket connection failed after select"));
			freeaddrinfo(result);
			closesocket(g_websocket);
			g_websocket = INVALID_SOCKET;
			WSACleanup();
	...
	{
		LogError(_T("Failed to send WebSocket upgrade request"));
		closesocket(g_websocket);
		g_websocket = INVALID_SOCKET;
		WSACleanup();
		return FALSE;
